...
for s in numpy.arange (2,10.5,0.2):
    szerokosci.append(round(s,1))
for g in numpy.arange(2,4.4,0.2):
    glebokosci.append(round(g,1))
...
n = len(glebokosci)
tabela = [cenabrutto[i * n:(i + 1) * n] for i in range((len(cenabrutto) + n - 1) // n )]
df = df.DataFrame(data=tabela, columns=glebokosci, index=szerokosci)
export_csv = df.to_csv(r'D:\Hubert\Cennik\COVERtabela160_2.csv',index=szerokosci,columns=glebokosci,header=True)
print(df)
print(cenabrutto)
